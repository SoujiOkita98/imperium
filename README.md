...
| [卷三·朝堂制衡](skill/references/03-朝堂制衡.md) | 多臣制衡 | 异源、异权、异时三道，防 multi-agent 结党、擅断与附和 |
...
| `异源` / `跨家族制衡` | 卷三：跨模型家族设议臣谏臣，防同党互证 |
| `异权` / `分封建制` / `切分权限` | 卷三：信息域 + 工具域二维切分，防擅断 |
| `异时` / `盲审` / `预承诺评议` | 卷三：Critic 在未见 Proposer 推理前预承诺立场 |
| `朝堂制衡` / `卷三` | 列出三异完整流程 |
